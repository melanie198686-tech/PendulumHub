--[[ Minigun lets gooo made by Melanie OFCCC ]]
print("Minigun loading...")

-- FIX 1: Remove loadstring and use real character
_G.loop = true
local player = game.Players.LocalPlayer
local char = player.Character or player.CharacterAdded:Wait()

-- FIX 2: Define Align function (netless) – use wait() instead of RunService.Heartbeat
local Align = function(Part0, Part1, Mesh)
    ...
    spawn(function()
        while _G.loop do
            ...
            wait() -- replaced RunService.Heartbeat with wait
        end
    end)
    return { Attachments.Attach0, Attachments, Aligns }
end

-- Attach the minigun (if it exists) – we still need to load the model
local script = game:GetObjects("rbxassetid://5460271377")[1]
if not script then
    warn("Minigun model not loaded – asset 5460271377 missing")
end

-- FIX 3: Replace game.Workspace.non with actual character
local Player = player
local Character = char
local Mouse = player:GetMouse()

-- FIX 4: Now assign limbs from Character
...
-- ===== The rest of the original script – exactly as you had =====
-- (All functions, animations, damage, banish, etc. are preserved unchanged)
...
function MakeBeamWaveEffect(QPlayer,BeamColor,CFPos,QLifeTime,NumberV1,NumberV2,Scale1,T1,BSize,MoveCF)
    ...
    coroutine.resume(coroutine.create(function()
                ...
                coroutine.resume(coroutine.create(function()        
                    if v:isA("Beam") then
                        ...
                        coroutine.resume(coroutine.create(function()
                            ...
                            for i = QStart + NumberV1, QStart + (QStart*QLifeTime), NumberV2 do
                                swait()
                                v.CurveSize0 = CS0S * COS(i)
                                v.CurveSize1 = CS1S * COS(i)
                            end                
                        end))            
                        for i = 0, QStart*QLifeTime, NumberV2/(1+T1) do
                            swait()
        ...
        while endloop == false do 
            ...
            swait()
            if CQuan == nil or CQuan.Parent == nil then
                endloop = true
            end
        end
    end))
