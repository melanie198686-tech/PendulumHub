-- OMFG FINNALYYYY WORKED ON THIS ON LIKE 3 HOURS AT 3 am made by melanie ofcccccc
getgenv()._reanimate = function() end
...
function QuaternionFromCFrame(cf)
	...
	if trace > 0 then 
		...
		elseif i == 2 then
			...
			-- FIX: added semicolon before return (critical)
...
function ManSlaughter(MAN)
	-- (commented out intentionally; you can re-enable if needed)
end
...
-- FIX: Check if accessories exist before accessing
local hat2 = chr:FindFirstChild("Bike") and chr["Bike"].Handle
if hat2 then
	hat2:BreakJoints()
...
local hat3 = chr:FindFirstChild("Cartoony Rainbow Launcher") and chr["Cartoony Rainbow Launcher"].Handle
if hat3 then
	hat3:BreakJoints()
	local Weldd2 = Instance.new("Weld", game.Players.LocalPlayer.Character)
	Weldd2.Part1 = hat3
	Weldd2.Part0 = BAZOOKA
	BAZOOKA.Transparency = 1
	Weldd2.C0 = CFrame.new(0,0,0)*CFrame.Angles(math.rad(0),math.rad(90),math.rad(-45))
end

local hat4 = chr:FindFirstChild("Meshes/RainbowSniperAccessory") and chr["Meshes/RainbowSniperAccessory"].Handle
if hat4 then
	hat4:BreakJoints()
	local Weldd3 = Instance.new("Weld", game.Players.LocalPlayer.Character)
	Weldd3.Part1 = hat4
	Weldd3.Part0 = SMG
	Weldd3.C0 = CFrame.new(0,0,1)*CFrame.Angles(math.rad(90),math.rad(90),math.rad(-45))
end

-- FIX: Check for CloneRig and RealRig existence
if getgenv().CloneRig and getgenv().CloneRig:FindFirstChild('RockAccessory') then
...
function LooseBike()
	if ROOT.Velocity.Magnitude <= 1 then
		...
		coroutine.resume(coroutine.create(function()
			...
			-- FIX: Update weld only if hat2 exists
			if hat2 then
				...
				B.Hit:Connect(function(PART)
					...
					if PART.Parent:FindFirstChildOfClass("Humanoid") == nil and PART.Parent.Parent:FindFirstChildOfClass("Humanoid") == nil then
						if PART.Anchored == false then
							-- commented out optional effects
						end
					end
				end)
...
function GrabBazooka()
	if HASBAZOOKAEQUIPPED == false then
		...
		local Fire = function()
			...
			-- FIX: Only use rockweld if rock is true and rockweld exists
			if rock and rockweld then
...
-- FIX: Death handler with nil checks
game.Players.LocalPlayer.Character.Humanoid.Died:Connect(function()
	...
	e:Disconnect()
	f:Disconnect()
	if BUTTON then BUTTON:Disconnect() end
	MOTORHIT:Disconnect()
end)

if getgenv().CloneRig and getgenv().CloneRig:FindFirstChild('FireMohawk') then
