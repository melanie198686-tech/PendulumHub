--[[ Original script – fixed: char reassignment, hair, and switched to wait() ]]
--[[ YESSSS MADE BY MELANIE!! ]]
warn("Netless Activated!")

_G.loop = true
local player = game.Players.LocalPlayer
local char = player.Character or player.CharacterAdded:Wait()  -- FIXED: use actual character
...
local Align = function(Part0, Part1, Mesh)
    ...
    if Mesh and m then m:Destroy() end
    Part0:BreakJoints()
    ...
    spawn(function()
        while _G.loop do
            ...
            wait()  -- FIXED: use wait() instead of RunService.Heartbeat
        end
    end)
    return { Attachments.Attach0, Attachments, Aligns }
end

-- Attach RedSaber if it exists, else we skip to avoid errors
local saberHandle = char:FindFirstChild("RedSaber") and char.RedSaber:FindFirstChild("Handle")
if saberHandle then
    local hat = Align(saberHandle, char['Right Arm'] or char:FindFirstChild("RightUpperArm"), false)
    local cf = (char['Right Arm'] or char:FindFirstChild("RightUpperArm")).CFrame * CFrame.new(0, -1, -2) * CFrame.Angles(math.rad(270), math.rad(-90), 0)
    hat[1].CFrame = cf:Inverse() * (char['Right Arm'] or char:FindFirstChild("RightUpperArm")).CFrame
    spawn(function()
        char.AncestryChanged:wait()
        _G.loop = false
    end)
else
    warn("No RedSaber found – skipping netless align")
end

local plrs = game:GetService("Players")
local plr = plrs.LocalPlayer
local Mouse = plr:GetMouse()
-- DO NOT reassign char = game.Workspace.non  -- REMOVED this line
...
local neck = tor and tor:FindFirstChild("Neck")
local rootj = root and root:FindFirstChild("RootJoint")
local LS = tor and tor:FindFirstChild("Left Shoulder")
local RS = tor and tor:FindFirstChild("Right Shoulder")
local LH = tor and tor:FindFirstChild("Left Hip")
local RH = tor and tor:FindFirstChild("Right Hip")
...
hairw.Part0 = head
hairw.Part1 = hair
hairw.C1 = CFrame.new(0, -0.1, 0)

-- ---- Saber model building (same as your original) ----
-- (I'm keeping it exactly as you had it, just copied here for completeness)
...
-- ---- Rest of original script (sounds, welds, functions) ----
...
function Sound(id, vol, par, name, looped)
    if not par:FindFirstChild(name) then
        ...
        sound.SoundId = id
        sound.Volume = vol
        if looped == true then sound.Looped = true end
        sound.Name = name
        sound:Play()
        return sound
    else
...
-- ---- Equip/Unequip, Swing, Choke (same as original) ----
...
-- ---- Animation loop (using wait instead of RunService) ----
while true do
    swait()
    sine = sine + 1
    ...
    -- Keep the breathing sound alive
