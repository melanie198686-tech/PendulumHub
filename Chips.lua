--[[
    Chips – FULLY FIXED (no external loadstring)
]]

local success, err = pcall(function()

    -- REMOVED the first loadstring (game:HttpGet...)
    -- The script defines its own clerp and quaternion functions, so it's not needed.

    -- Wrap the second loadstring safely
    pcall(function()
    ...
    -- ===========================
    -- FIXED ALIGN (no velocity spam)
    -- ===========================
    local function Align(Part0, Part1, Mesh)
        local Aligns = {
        ...
        spawn(function()
            while _G.loop do
                local mag = (Part0.Position - (Part1.CFrame * Attachments.Attach0.CFrame:Inverse()).p).magnitude
                if mag >= 5 then
                    Part0.CFrame = Part1.CFrame * Attachments.Attach0.CFrame:Inverse()
                end
                -- REMOVED the velocity line that caused weird movement
    ...
    -- Safe pillow alignment
    local pillow = char:FindFirstChild("Pillow")
    if pillow and pillow:FindFirstChild("Handle") then
        local hat = Align(pillow.Handle, char['Right Arm'], false)
        local cf = char['Right Arm'].CFrame * CFrame.new(0,-1,0) * CFrame.Angles(math.rad(35),0,0)
    ...
    -- ===========================
    -- SAFE LIMB DETECTION (R6/R15)
    -- ===========================
    local hum = char:FindFirstChildOfClass('Humanoid') or error("No Humanoid")
    local hed = char:FindFirstChild("Head") or error("No Head")
    local root = char:FindFirstChild('HumanoidRootPart') or error("No HumanoidRootPart")
    local rootj = root.RootJoint
    local tors = char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso") or error("No Torso")
    local ra = char:FindFirstChild("Right Arm") or char:FindFirstChild("RightUpperArm") or error("No Right Arm")
    local la = char:FindFirstChild("Left Arm") or char:FindFirstChild("LeftUpperArm") or error("No Left Arm")
    local rl = char:FindFirstChild("Right Leg") or char:FindFirstChild("RightLowerLeg") or error("No Right Leg")
    local ll = char:FindFirstChild("Left Leg") or char:FindFirstChild("LeftLowerLeg") or error("No Left Leg")
    local neck = tors:FindFirstChild("Neck") or tors:FindFirstChild("NeckR15") or error("No Neck")
    ...
    -- FIXED rayCast: uses safe `char` as default Ignore
    function rayCast(Position, Direction, Range, Ignore)
        return game:service("Workspace"):FindPartOnRay(Ray.new(Position, Direction.unit * (Range or 999.999)), Ignore or char)
    end
    ...
    if Player_Size ~= 1 then
        ...
        RW.C0 = CF(1.5 * Player_Size, 0.5 * Player_Size, 0 * Player_Size) * angles(Rad(0), Rad(0), Rad(0))
        LW.C0 = CF(-1.5 * Player_Size, 0.5 * Player_Size, 0 * Player_Size) * angles(Rad(0), Rad(0), Rad(0))
...
if not success then
    warn("CHIPS ERROR: " .. tostring(err))
    print("Script error caught – no crash.")
end
